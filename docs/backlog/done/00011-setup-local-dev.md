...
- [x] `just dev-local` démarre tout (DB + backend + frontend) en < 10s
- [x] `just dev-local-stop` arrête proprement tous les services
...
- [x] Documentation à jour (justfile)
...
2. Configurer `DATABASE_URL=postgresql+psycopg://...`
...
- Test Playwright: création de room OK (room créée avec succès)

### Usage
```bash
just dev-local       # Démarre tout en < 10s
just dev-local-stop  # Arrête tout
